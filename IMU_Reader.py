...
if __name__ == '__main__':
  ...
  while True:
    ...
    # Desired format: Date & Time | MagX | MagY | MagZ | AccX | AccY | AccZ | GyroX | GyroY | GyroZ | Temp | Humid | Roll | Pitch | Yaw
    # Note: We have since removed Humidity from the row above (and its IMU data)

    # Current date and time (rounded millseconds to 3 decimal places):
    current_dt = datetime.datetime.now()
    string_dt = current_dt.strftime('%Y-%m-%d %H:%M:%S.%f')
    string_dt = string_dt[:-3]

    # Magnetic parameters (rounded to 3 decimal places):
    mag_x = str(round(Mag[0], 3))
    mag_y = str(round(Mag[1], 3)) 
    mag_z = str(round(Mag[2], 3))

    # Acceleration parameters (rounded to 3 decimal places):
    acc_x = str(round(Accel[0]/16384, 3)) # Converting from LSB form to G
    acc_y = str(round(Accel[1]/16384, 3)) # Converting from LSB form to G
    acc_z = str(round(Accel[2]/16384, 3)) # Converting from LSB form to G

    # Gyroscope parameters (rounded to 3 decimal places):
    gyro_x = str(round(Gyro[0]/32.8, 3)) # Converting from LSB form to deg/s
    gyro_y = str(round(Gyro[1]/32.8, 3)) # Converting from LSB form to deg/s
    gyro_z = str(round(Gyro[2]/32.8, 3)) # Converting from LSB form to deg/s

    # Temperature and humidity (rounded to 2 decimal places):
    current_temp = str(round(shtc3.SHTC3_Read_Temperature(), 2)) # In degrees celcius
    #current_humid = str(round(shtc3.SHTC3_Read_Humidity(), 2)) # As a % (REMOVED as the IMU on the SHARC Buoy does not have a humidity sensor, so cannot compare)

    # Pitch, roll, and yaw parameters
    pitch = str(round(math.asin(-2 * q1 * q3 + 2 * q0* q2)* 57.3, 2))
    roll  = str(round(math.atan2(2 * q2 * q3 + 2 * q0 * q1, -2 * q1 * q1 - 2 * q2* q2 + 1)* 57.3, 2))
    yaw   = str(round(math.atan2(-2 * q1 * q2 - 2 * q0 * q3, 2 * q2 * q2 + 2 * q3 * q3 - 1) * 57.3, 2))

    current_read = string_dt + " " + mag_x + " " + mag_y + " " + mag_z + " " + acc_x + " " + acc_y + " " + acc_z + " " + gyro_x + " " + gyro_y + " " + gyro_z + " " + current_temp + " " + pitch + " " + roll + " " + yaw
